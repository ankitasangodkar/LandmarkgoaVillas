...
/**
 * Class Pado_Vc_Templates_Editor
 * @since 1.0
 */
class Pado_Vc_Templates_Editor {
    /**
     ...
     * @var bool
     */
    protected $pado_templates = false;
    /**
     * @since 4.4
    ...
    public function addTemplatesTab( $data ) {
        $newCategory = array(
            'category'        => 'pado_templates',
            'category_name'   => esc_html__( 'Pado Templates', 'pado' ),
            'category_weight' => 1,
    ...
    protected function get_template_categories() {
        ...
        $categories = array(
            'all'         => esc_html__( 'All', 'pado' ),
            'banners'      => esc_html__( 'Banners', 'pado' ),
            'about'      => esc_html__( 'About', 'pado' ),
            'skills'    => esc_html__( 'Skills', 'pado' ),
            'faq'         => esc_html__( 'FAQ', 'pado' ),
            'contacts'    => esc_html__( 'Contacts', 'pado' ),
            'services'     => esc_html__( 'Services', 'pado' ),
            'info-block'  => esc_html__( 'Info Block', 'pado' ),
            'team'        => esc_html__( 'Team', 'pado' ),
            'testimonials' => esc_html__( 'Testimonials', 'pado' ),
            'video'        => esc_html__( 'Video', 'pado' ),
            'coming-soon'        => esc_html__( 'Coming Soon', 'pado' ),
            'headings'        => esc_html__( 'Headings', 'pado' ),
            'portfolio'        => esc_html__( 'Portfolio', 'pado' ),
            'line-of-images'        => esc_html__( 'Line of images', 'pado' ),
            'posts'        => esc_html__( 'Posts', 'pado' ),
            'accordion'        => esc_html__( 'Accordion', 'pado' ),
            'buttons'        => esc_html__( 'Buttons', 'pado' ),
            'gallery'        => esc_html__( 'Gallery', 'pado' ),
            'image-parallax'        => esc_html__( 'Image parallax', 'pado' ),
            'map'        => esc_html__( 'Map', 'pado' ),
            'products'        => esc_html__( 'Products', 'pado' ),
            'promotions'        => esc_html__( 'Promotions', 'pado' ),
            'sliders'        => esc_html__( 'Sliders', 'pado' ),
        );
    ...
    public function renderTemplateBlock( $category ) {
        ...
        if ( 'pado_templates' === $category['category'] ) {

            $category['output'] = '<div class="vc_col-md-2 pado-sorting-container">';
            $category['output'] .= $this->get_template_categories();
            $category['output'] .= '</div>';


            $category['output'] .= '
			<div class="vc_column vc_col-sm-12 pado-templates-container">
    ...
    function renderTemplateWindow( $template_name, $template_data ) {
        if ( 'pado_templates' === $template_data['type'] ) {
            return $this->renderTemplateWindowPadoTemplates( $template_name, $template_data );
        }
     ...
     * @return string
     */
    public function renderTemplateWindowPadoTemplates( $template_name, $template_data ) {
        ob_start();
        ...
        $preview_template_title = esc_attr( 'Preview template', 'pado' );
        $add_template_title = esc_attr( 'Add template', 'pado' );
...
HTML;
    ...
    public function renderBackendTemplate() {
        ...
        if ( ! isset( $template_id, $template_type ) || '' === $template_id || '' === $template_type ) {
            die( 'Error: Vc_Pado_Templates::renderBackendTemplate:1' );
        }
    ...
    public function getAllTemplates() {
        ...
        $data = array();
        $pado_templates = $this->getTemplates();
        $category_templates = array();
        foreach ( $pado_templates as $template_id => $template_data ) {
            $category_templates[] = array(
    ...
    public function loadDefaultTemplates() {
        ...
        if ( ! is_array( $this->pado_templates ) ) {
            $this->pado_templates = $this->allTemplates();
        }

        return $this->pado_templates;
    }
    ...
    public function getDefaultTemplate( $template_index ) {
        ...
        if ( ! is_numeric( $template_index ) || ! is_array( $this->pado_templates ) || ! isset( $this->pado_templates[ $template_index ] ) ) {
            return false;
        }

        return $this->pado_templates[ $template_index ];
    }
    ...
    public function addDefaultTemplates( $data ) {
        if ( is_array( $data ) && ! empty( $data ) && isset( $data['name'], $data['content'] ) ) {
            if ( ! is_array( $this->pado_templates ) ) {
                $this->pado_templates = array();
            }
            $this->pado_templates[] = $data;
    ...
    public function renderTemplateListItem( $template ) {
        $name = isset( $template['name'] ) ? esc_html( $template['name'] ) : esc_html( __( 'No title', 'pado' ) );
...
HTML;
        $output .= '<div class="pado-template-preview">';
        if ( $new ) { $output .= '<span class="pado-badge-new">New</span>'; }
        ...
        $output .= '<span class="pado-template-categories">' . esc_html( $template_sort_name ) . '</span>';
        $output .= <<<HTML
						</div>
