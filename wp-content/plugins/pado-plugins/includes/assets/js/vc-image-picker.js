...
if (function($) {
        var padoResponsiveAlignmentParam = Backbone.View.extend({
            events: {},
        ...
        // VC pado responsive aligment js
        vc.atts.responsive_alignment = {
            parse: function(param) {
                return this.content().find("input.wpb_vc_param_value." + param.param_name).data("padoResponsiveAlignment").save().join(" ")
            },
            init: function(param, $field) {
                $('[data-column-offset="true"]', $field).each(function() {
                    var $this = $(this);
                    $this.find(".wpb_vc_param_value").data("padoResponsiveAlignment", new padoResponsiveAlignmentParam({
                        el: $this
                    }).render())
                })
            }
...
(function($) {
    ...
    vc.atts.gradient = {
        ...
        init: function(param, $field) {
            ...
            var defaults = {
                ...
                targetInputElement: $field.find('.pado-gradient-css'),
                targetBgInputElement: $field.find('.pado-gradient-bg'),
                startingGradient: startingGradient,
                startingBgProps: startingBgProps
            };

            var options = $.extend(true, {}, defaults, param.settings);
            $field.find('.pado-gradient').icsge(options);
        },

        parse: function(param) {
            var $field = this.content().find(".wpb_vc_param_value[name=" + param.param_name + "]"),
                $css = $field.next('.pado-gradient-css'),
                $bg = $css.next('.pado-gradient-bg');

            return $css.val() + '|' + $bg.val();
        }
    };

    // VC pado slider parameter js
    vc.atts.pado_slider = {
        ...
        init: function( param, $field ) {
            ...
            var $handle = $field.find('.pado-handle');

            $field.find('.pado-slider').slider({
                min: param.min,
                max: param.max,
                step: param.step,
                value: $init_val,
    ...
    vc.atts.pado_shape_divider = {
        ...
        init: function(param, $field) {
            $(".pado_color-control", $field).each(function() {
            ...
            $( ".pado-slider", $field).each( function() {
                var $init_val, $control = $( this ),
                    value  = $control.parent().find( ".pado-sliderinput").val(),
                    $handle = $control.find('.pado-handle');
                ...
                $control.slider({
                    ...
                    change: function( event, ui ) {
                        $control.parent().find( ".pado-sliderinput").val( ui.value );
                    }
                });
            });

            $('h3.pado-shape-divider-heading', $field).click(function(e) {
                $(this).next('.pado-inner-wrap').removeClass('hidden')
                    .parent().siblings().children('.pado-inner-wrap').addClass('hidden');
            });

            $(document).on('change', '.pado_shape_divider-type', function() {
    ...
    vc.atts.responsive_css_editor = {
        ...
        init: function(param, $field) {
            $('h3.pado-responsive-css-heading', $field).click(function(e) {
                $(this).parent('.pado-main-responsive-wrapper').addClass('active').siblings().removeClass('active');
            });
        },

    };

    // VC Advanced pado Checkbox js
    vc.atts.pado_advanced_checkbox = {

        parse: function(param) {

            if ($(".pado-advanced-checkbox input").is(':checked')) {
                var $checked = 'checked="checked"';
            } else {
                var $checked = '';
            }

            var $pado_panel = $('#vc_ui-panel-edit-element'),
                $easy_mode_checkbox = $('<label class="pado-easy-mode-switch"><input type="checkbox" class="pado-easy-mode-checkbox" ' + $checked + ' value="true">Easy Mode<span><i class="fa fa-power-off"> </i></span></label>'),
                $easy_mode;

            $pado_panel.find('.pado-easy-mode-switch').remove();
            $pado_panel.find('.vc_ui-panel-header-controls').prepend($easy_mode_checkbox);
            $easy_mode = $easy_mode_checkbox.find('.pado-easy-mode-checkbox');

            var $advanced_checkbox = $(".pado-advanced-checkbox input");
    ...
    // run pado functions after shortcode options changed in frontend editor
    ...
    //
    // 	if ( ev === "update" && typeof padoMaps !== typeof undefined ) {
    // 		$('[data-plugin-map]').padoMaps();
    // 	};
    //
    // });
    ...
    // 	if ( ev === "update" && typeof padoParallax !== typeof undefined ) {
    // 		$('[data-parallax]').padoParallax();
    // 	};
    //
    // });


    function templates_pado() {
        if(jQuery('.pado-templates-container').length){
            jQuery('.pado-templates-container > .vc_ui-list-bar').each(function () {
                var layoutM = 'masonry';
                jQuery(this).isotope({
    ...
    vc.TemplateWindowUIPanelBackendEditor = vc.TemplatesPanelViewBackend.vcExtendUI(vc.HelperPanelViewHeaderFooter).vcExtendUI(vc.HelperTemplatesPanelViewSearch).extend({
        ...
        show: function() {
            ...
            this.clearSearch(), vc.TemplateWindowUIPanelBackendEditor.__super__.show.call(this), $('.vc_edit-form-tab[data-tab="pado_templates"] .sortable_templates ul > li').each(function() {
                "all" == $(this).attr("data-sort") ? $(this).find(".count").html($('.vc_edit-form-tab[data-tab="pado_templates"] .vc_ui-template-list > .vc_ui-template').length) : $(this).find(".count").html($('.vc_edit-form-tab[data-tab="pado_templates"] .vc_ui-template-list > .vc_ui-template.' + $(this).attr("data-sort")).length)
            }), $('.vc_edit-form-tab[data-tab="pado_templates"] .sortable_templates li[data-sort="all"]').addClass("active").trigger("click"), $('.vc_edit-form-tab[data-tab="pado_templates"] .sortable_templates li').click(function() {
                $('.vc_edit-form-tab[data-tab="pado_templates"] .sortable_templates li').removeClass("active"), $(this).addClass("active");
                var t = $(this).attr("data-sort");
                $('.vc_edit-form-tab[data-tab="pado_templates"] .vc_ui-template-list > .vc_ui-template').removeClass("hidden"), "all" != t && $('.vc_edit-form-tab[data-tab="pado_templates"] .vc_ui-template-list > .vc_ui-template:not(.' + t + ")").addClass("hidden")

                setTimeout(templates_pado,0);
            }),
            ...
            //
            // jQuery('.pado-templates-container img').on('load', function () {
            //
            //     alert('hi');
        ...
        showMessageDisabled: !1,
        show: function() {
            this.clearSearch(), vc.TemplateWindowUIPanelFrontendEditor.__super__.show.call(this), $('.vc_edit-form-tab[data-tab="pado_templates"] .sortable_templates ul > li').each(function() {
                "all" == $(this).attr("data-sort") ? $(this).find(".count").html($('.vc_edit-form-tab[data-tab="pado_templates"] .vc_ui-template-list > .vc_ui-template').length) : $(this).find(".count").html($('.vc_edit-form-tab[data-tab="pado_templates"] .vc_ui-template-list > .vc_ui-template.' + $(this).attr("data-sort")).length)
            }), $('.vc_edit-form-tab[data-tab="pado_templates"] .sortable_templates li[data-sort="all"]').addClass("active").trigger("click"), $('.vc_edit-form-tab[data-tab="pado_templates"] .sortable_templates li').click(function() {
                $('.vc_edit-form-tab[data-tab="pado_templates"] .sortable_templates li').removeClass("active"), $(this).addClass("active");
                var t = $(this).attr("data-sort");
